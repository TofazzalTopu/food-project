...
(function($){
	...
	$.fn.decreaseCount = function(pCount) {
		...
		this.each( function() {
			...
			var p = this.p;
			if (p.currentCount == false)
			{
				var rPage = parseInt(p.rp);
				if (p.total < rPage)
				{
					p.currentCount = p.total;
				} else {
					p.currentCount = rPage;
				}
			}
			...
			stat = stat.replace(/{from}/,r1);
			stat = stat.replace(/{to}/,p.currentCount);
			stat = stat.replace(/{total}/,p.total);
		});
        if (stat != false) {
		    $('.pPageStat',this.pDiv).html(stat);
