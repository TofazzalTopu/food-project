 ...
 */
@Component("lockUserAccountAction")
class LockUserAccountAction implements IAction {
    @Autowired
    ...
    Object execute(params, object) {
        List errorList = []
        try {
            if(userAccountService.lock((Map)object)){
        ...
        return UserMessageBuilder.createMessage(userAccountService.getUserMessage("ApplicationUser.title", null), Message.SUCCESS, "Application User is locked successfully")
    }
