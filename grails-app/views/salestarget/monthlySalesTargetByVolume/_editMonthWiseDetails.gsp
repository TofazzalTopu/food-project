...
//        resetMonthWiseTarget();
        ...
        var actionUrl = "";
        <g:if test="${yearlySalesTargetByVolume.isSalesManIncluded}">
        ...
        jqGridMonthlyAssignedTargetList = $("#jqgrid-grid-subordinate-monthly").jqGrid({
            ...
            loadComplete: function (data) {
                $.each(data.rows, function (i, item) {
                    var rowData = jqGridMonthlyAssignedTargetList.getRowData(data.rows[i].id);
                    var subOrdinateRow = jqGridSubordinateEmployeeList.getRowData(rowData.employeeId);
                    if(!subOrdinateRow.id){
                        MessageRenderer.renderErrorText("You are no longer supervisor for Subordinate: [" + rowData.employeeCode + "] " + rowData.employeeName);
                        isRedistributionNeeded = true;
                    }else{
                        jqGridSubordinateEmployeeList.setSelection(rowData.employeeId, true);
                    }
                });
            },
            onSelectRow: function(rowid, status) {
