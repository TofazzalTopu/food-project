...
<script type="text/javascript" language="Javascript">
    ...
    $(document).ready(function() {
      ...
      /*  $('input:radio[id="ship"]').change(
                function(){
//                    alert($(this).val())
//                    alert("ship "+ship)
                    if ($(this).is(':checked') && $(this).val() == 'ShipAddress') {
                        alert($(this).val())
                        alert("ship "+ship)
                        own = '';
//                        ship = 'ship'
...
*/
       ...
       $("#jqgrid-grid-finishProduct").jqGrid({
            ...
            colModel: [
                ...
                {name: 'mainName', index: 'pmainName', width: 200, align: 'left', hidden:true},
                {name: 'productName', index: 'productName', width: 200, align: 'left'},
    ...
    function executeAjaxNewPrimaryDemandOrder() {
        ...
        if(isShip){
            allData["shippingAddressId"] = $("#shippingAddress").val();
            allData["shipTo"] = 'ship';
...
//            $("#jqgrid-grid").trigger("reloadGrid");
    ...
    var CustomerInfo = {
        ...
        setCustomerInformation: function(customerCoreInfoId, customerCoreInfo){
            ...
            readCustomerBalanceAndShippingAddress(customerCoreInfoId);
...
//            SubmissionLoader.showTo();
    ...
    function loadProduct(customerId) {
        ...
        jQuery('#searchKey').autocomplete({
            ...
            source:function (request, response) {
                var data = {searchKey:request.term};
                var url = '${resource(dir:'secondaryDemandOrder', file:'listProduct')}?id=' + customerId + "&query=" + $('#searchKey').val();
                DocuAutoComplete.setSpinnerSelector('searchKey').execute(response, url, data, function (item) {
                    item['label'] = "[" + item['code'] + "] " + item['name'];
                    item['value'] = item['label'];
                    return item;
