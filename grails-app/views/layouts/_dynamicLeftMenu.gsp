...
<div class="left_nav">
    <div id="left-menu-accordion">
        ...
        <% levelThreeMenuList = menuGenerator.getLevelWorkflowMenu() %>
        <g:if test="${levelThreeMenuList && levelThreeMenuList.size() > 0}">
            <h3>
                <a href="javascript:void(0)" tabindex="-1">Approval</a>
            </h3>
            <div class="left-menu-section-items">
                ...
                <g:if test="${levelThreeMenuList.size() > 1}">
                    <g:each in="${levelThreeMenuList}" var="featureAction">
                        <div>
                            ...
                            %{--urlParts = featureAction?.menuUrl?.split("/")--}%
                            %{--%>--}%
                            <a href="${request.contextPath}/primaryDemandOrder/showPrimaryDemandOrderForApprove" id="abc">
                                <span class="icon">
                                    <img src="${request.contextPath}/images/leftmenu/leftmenu_blank.png" alt="icon" class="icon"/>
                                </span>
                                <span class="menuTxtSpan">${featureAction.name}-${featureAction.position}</span>
                            </a>
                        </div>
                    </g:each>
                </g:if>
...
//                        urlParts = first?.menuUrl?.split("/")
                        %>
                        <a href="${request.contextPath}/primaryDemandOrder/showPrimaryDemandOrderForApprove" >
                            <img src="${request.contextPath}/images/leftmenu/leftmenu_blank.png" alt="icon" class="icon"/>
                            ${first?.name}
                        </a>
                    </div>
