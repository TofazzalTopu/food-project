        ...
        var data = ${aaData}
                customerDataTable = $('#customer_${id}').dataTable({
                    ...
                    "fnRowCallback":function (nRow, aData, iDisplayIndex) {
                        ...
                        var cusCat = aData.category;
                        var name = "[" + cusCode + "] " + cusName;
                        $('td:eq(0)', nRow).html('<a href="javascript:void(0)" onclick="loadDataInField(' + '\'' + aData.id + '\', \'' + name + '\', \'' + cusName + '\', \'' + cusCode + '\', \'' + cusCat + '\')">' + aData.code + '</a>');
                        return nRow;
                    }
                });
    }
    function loadDataInField(customerCodeInfoId, name, cusName, cusCode, cusCat) {
        $.fancybox.close();
        if('${sale}' == '') {
            CustomerInfo.setCustomerInformation(customerCodeInfoId, cusName, cusCode, cusCat);
        }else{
            CustomerInfo.setSecCustomerInformation(customerCodeInfoId, cusName, cusCode, cusCat);
        }
//        loadProduct(customerCodeInfoId);
//        readCustomerBalanceAndShippingAddress(customerCodeInfoId);
