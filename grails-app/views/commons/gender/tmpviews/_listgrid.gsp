<div class="clear"></div>
<div class="width800">
    <div class="jqgrid-container blue_grid">
        <table id="jqgrid-grid-gender"></table>

        <div id="jqgrid-pager-gender"></div>
    </div>
    %{--<div id="dialog-confirm-gender" title="Delete alert" style="display: none">--}%
    %{--<p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>These item(s) will be permanently deleted and cannot be recovered. Are you sure?</p>--}%
    %{--</div>--}%
</div>
<g:render
        template='../commons/gender/tmpscripts/listscript'/>


