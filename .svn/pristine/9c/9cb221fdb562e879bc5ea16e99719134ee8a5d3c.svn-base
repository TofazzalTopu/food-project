...
@Component("updateReceiveProductsFromMarketAction")
class UpdateReceiveProductsFromMarketAction implements IAction {
  public static final Log log = LogFactory.getLog(UpdateReceiveProductsFromMarketAction.class)
  private final String MESSAGE_HEADER = 'receiveProductsFromMarket.header'
  private final String MESSAGE_SUCCESS = 'receiveProductsFromMarket.update.success'
  ...
  public Object preCondition(def params) {
    ...
    try {
        ...
        if (!receiveProductsFromMarket.validate()) {
            return UserMessageBuilder.createMessage(receiveProductsFromMarketService.getUserMessage(MESSAGE_HEADER, null), Message.ERROR, receiveProductsFromMarketService.getErrorMessage(receiveProductsFromMarket), receiveProductsFromMarket)
        }
  ...
  public Object execute(def params, def object) {
        ...
        try {
            ...
            receiveProductsFromMarketService.update(receiveProductsFromMarket)
        } catch (Exception ex) {
            log.error(ex.message)
            return UserMessageBuilder.createMessage(receiveProductsFromMarketService.getUserMessage(MESSAGE_HEADER, null), Message.ERROR, receiveProductsFromMarket, ex)
        }
