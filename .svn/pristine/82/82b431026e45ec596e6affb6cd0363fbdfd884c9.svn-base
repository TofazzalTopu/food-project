...
@Component("updateUrlWrapperAction")
class UpdateUrlWrapperAction implements IAction {
  public static final Log log = LogFactory.getLog(UpdateUrlWrapperAction.class)
  private final String MESSAGE_HEADER = 'urlWrapper.header'
  private final String MESSAGE_SUCCESS = 'urlWrapper.update.success'
  ...
  public Object execute(def params, def object) {
        ...
        try {
            ...
            urlWrapperService.update(urlWrapper)
        } catch (Exception ex) {
            log.error(ex.message)
            return UserMessageBuilder.createMessage(urlWrapperService.getUserMessage(MESSAGE_HEADER, null), Message.ERROR, urlWrapper, ex)
        }
