...
@Component("createMonthlySalesTargetByAmountAction")
class CreateMonthlySalesTargetByAmountAction extends Action{
    public static final Log log = LogFactory.getLog(CreateMonthlySalesTargetByAmountAction.class)
    private final String MESSAGE_HEADER = 'New Sales Target By Amount'
    private final String MESSAGE_SUCCESS = 'Sales Target By Amount Created Successfully'
    Message message
    ...
    public Object execute(def params, def object) {
        try {
            ...
            YearlySalesTargetByAmount yearlySalesTargetByAmount = YearlySalesTargetByAmount.findByEmployeeAndTargetYear(currentEmployee, targetYear)
            if(!yearlySalesTargetByAmount){
                yearlySalesTargetByAmount = new YearlySalesTargetByAmount(employee: currentEmployee, targetYear: targetYear, targetAmount: targetAmount)
                yearlySalesTargetByAmount.startDate = DateUtil.getSimpleDateWithFormat("01-01-" + targetYear.toString(), 'dd-MM-yyyy')
                yearlySalesTargetByAmount.endDate = DateUtil.getSimpleDateWithFormat("31-12-" + targetYear.toString(), 'dd-MM-yyyy')
                yearlySalesTargetByAmount.userCreated = applicationUser
            }else{
            ...
            for(int j= 0; j <12; j++){
                int targetMonth = j+1
                MonthlySalesTargetByAmount selfMonthlySalesTargetByAmount = MonthlySalesTargetByAmount.findWhere(employee: currentEmployee, targetYear: targetYear, targetMonth: targetMonth)
                if(selfMonthlySalesTargetByAmount){
                    selfMonthlySalesTargetByAmount.userUpdated = applicationUser
                    selfMonthlySalesTargetByAmount.isActive = true
                }else{
                    selfMonthlySalesTargetByAmount = new MonthlySalesTargetByAmount(targetYear: targetYear, employee: currentEmployee, targetMonth: targetMonth, isActive: true)
                    String key = params.get("selfMonthWiseTarget_" + j.toString() + "_targetAmount")
                    selfMonthlySalesTargetByAmount.targetAmount = Float.parseFloat(key)
                    selfMonthlySalesTargetByAmount.startDate =  DateUtil.firstDayOfMonth(targetYear, targetMonth)
                    selfMonthlySalesTargetByAmount.endDate = DateUtil.lastDayOfMonth(targetYear, targetMonth)
                    selfMonthlySalesTargetByAmount.userCreated = applicationUser
                    selfMonthlySalesTargetByAmount.yearlySalesTargetByAmount = yearlySalesTargetByAmount
                }
                if (!selfMonthlySalesTargetByAmount.validate()) {
                    message = this.getValidationErrorMessage(selfMonthlySalesTargetByAmount)
                    throw new RuntimeException(message.messageBody[0].toString())
                }
            ...
            for(int i = 0; i < employeeCount; i++){
                ...
                for(int k = 0; k < 12; k++){
                    ...
                    if (!monthlySalesTargetByAmount.validate()) {
                        message = this.getValidationErrorMessage(monthlySalesTargetByAmount)
                        throw new RuntimeException(message.messageBody[0].toString())
                    }
                    ...
                    for(int day = 1; day <= maxDayOfMonth; day++){
                        ...
                        if (!dailySalesTargetByAmount.validate()) {
                            message = this.getValidationErrorMessage(dailySalesTargetByAmount)
                            throw new RuntimeException(message.messageBody[0].toString())
                        }
                ...
                subordinateYearlySalesTargetByAmount.targetAmount = subordinateYearlyTarget

                if (!subordinateYearlySalesTargetByAmount.validate()) {
                    message = this.getValidationErrorMessage(subordinateYearlySalesTargetByAmount)
                    throw new RuntimeException(message.messageBody[0].toString())
                }
                subordinateYearlySalesTargetByAmountList.add(subordinateYearlySalesTargetByAmount)
            }
            ...
            monthlySalesTargetByAmountService.createNewSalesTarget(mapInstance)
