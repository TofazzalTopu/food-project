<script type="text/javascript" language="Javascript">
    $(document).ready(function () {
        ...
        getBusinessMonth();
        $("#gFormBusinessHoliday").validationEngine({   //    client side validation
            isOverflown: true,
            overflownDIV: ".ui-layout-center"
        });
        $("#gFormBusinessHoliday").validationEngine('attach');
        reset_form("#gFormBusinessHoliday");
        $("#jqgrid-grid-division").jqGrid({
            ...
            viewrecords: true,
            sortorder: "desc",
            caption: "All Business Holiday Information",
            autowidth: true,
            autoheight: true,
            scrollOffset: 0,
            altRows: true,
    ...
    function getSelectedDivisionId() {
        ...
        if (divisionId == null) {
            divisionId = $('#gFormBusinessHoliday input[name = id]').val();
        }
        return divisionId;
    }
    function executePreConditionDivision() {
        ...
        trim_form();
        if ($("#gFormBusinessHoliday").validate().form({onfocusout: false}) == false) {
            return false;
        }
        return true;
    }
    function executeAjaxHoliday() {
        if ($('#month').val() && $('#day').val()) {
            ...
            var actionUrl = null;
            if ($('#gFormBusinessHoliday input[name = id]').val()) {
            ...
            jQuery.ajax({
                type: 'post',
                data: jQuery("#gFormBusinessHoliday").serialize(),
                url: actionUrl,
            ...
            MessageRenderer.showHeaderMessage("Please select Month And Day", 0);
        }
    ...
    function showHoliday(entity) {
        $('#gFormBusinessHoliday input[name = id]').val(entity.id);
        $('#gFormBusinessHoliday input[name = version]').val(entity.version);
    ...
    function executePostConditionForSearchDivision(data, fieldName, fieldValue) {
        if (data == null) {
            reset_form("#gFormBusinessHoliday"); // Clear Form
                ...
                buttons: {
                    'Delete item(s)': function() {
                        ...
                        $.ajax({
                            ...
                            dataType: "json",
                            data:jQuery("#gFormBusinessHoliday").serialize(),
                            url: "${resource(dir:'businessHoliday', file:'delete')}",
                            success: function(message) {
