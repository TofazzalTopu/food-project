...
<div>
        ...
        <table class="simple-table-css">
            ...
            <g:each in="${productList}" status="i" var="product">
                <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                    <td class="width30"><g:set var="sl" value="${sl + 1}"/>${sl}</td>
                    ...
                    <g:if test="${categoryList}">
                        <g:each in="${categoryList}" var="category">
                            ...
                            <g:if test="${category.id == ApplicationConstants.PRICING_CATEGORY_NEGOTIATED_ID}">
                                <td class="negotiated width40"><g:textField class="width40 alin_right amount baseData negotiated"
                                ...
                                <td class="negotiated width40"><g:textField class="width30 amount negotiated"
                                ...
                                <td class="negotiated width40"><g:textField class="width30 alin_right amount negotiated"
                                ...
                                <td class="negotiated width40"><g:textField class="width40 alin_right amount negotiated"
                                                                                    name="actualPrice_${category.id}_${product.id}"
                                                                                    id="actualPrice_${category.id}_${product.id}" value=""
                                                                                    readonly="readonly"/></td>
                            </g:if>
                            <g:else>
                                <td class="width40"><g:textField class="width40 alin_right amount baseData"
                                                                 name="basePrice_${category.id}_${product.id}"
                                                                 id="basePrice_${category.id}_${product.id}" value=""
                                                                 maxlength="9" onblur="calculate(this.id, '${category.id}_${product.id}', 'basePrice')"/></td>
                                <td class="width40"><g:textField class="width30 amount"
                                                                 name="vatPercent_${category.id}_${product.id}"
                                                                 id="vatPercent_${category.id}_${product.id}" value=""
                                                                 maxlength="5" onblur="calculate(this.id, '${category.id}_${product.id}', 'vatPercent')"/></td>
                                <td class="width40"><g:textField class="width30 alin_right amount"
                                                                 name="vatAmount_${category.id}_${product.id}"
                                                                 id="vatAmount_${category.id}_${product.id}" value=""
                                                                 maxlength="8" onblur="calculate(this.id, '${category.id}_${product.id}', 'vatAmount')"/></td>
                                <td class="width40"><g:textField class="width40 alin_right amount"
                                                                 name="actualPrice_${category.id}_${product.id}"
                                                                 id="actualPrice_${category.id}_${product.id}" value=""
                                                                 readonly="readonly"/></td>
                            </g:else>
    ...
    <g:if test="${savedPriceList}">
        <g:each in="${savedPriceList}" var="savedPrice">
            <script type="text/javascript">
                $("#basePrice_" + ${savedPrice.pricingCategoryId} + "_" + ${savedPrice.finishProductId}).val(${savedPrice.price});
                $("#vatPercent_" + ${savedPrice.pricingCategoryId} + "_" + ${savedPrice.finishProductId}).val(${savedPrice.vatPercentage});
                $("#vatAmount_" + ${savedPrice.pricingCategoryId} + "_" + ${savedPrice.finishProductId}).val(${savedPrice.vatAmount});
                $("#actualPrice_" + ${savedPrice.pricingCategoryId} + "_" + ${savedPrice.finishProductId}).val(${savedPrice.totalAmount});
            </script>
        </g:each>
    </g:if>
</div>
