...
@Component("updateCodeGenerationFormatAction")
class UpdateCodeGenerationFormatAction implements IAction {
  public static final Log log = LogFactory.getLog(UpdateCodeGenerationFormatAction.class)
  private final String MESSAGE_HEADER = 'codeGenerationFormat.header'
  private final String MESSAGE_SUCCESS = 'codeGenerationFormat.update.success'
  ...
  public Object execute(def params, def object) {
        ...
        try {
            ...
            codeGenerationFormatService.update(codeGenerationFormat)
        } catch (Exception ex) {
            log.error(ex.message)
            return UserMessageBuilder.createMessage(codeGenerationFormatService.getUserMessage(MESSAGE_HEADER, null), Message.ERROR, codeGenerationFormat, ex)
        }
