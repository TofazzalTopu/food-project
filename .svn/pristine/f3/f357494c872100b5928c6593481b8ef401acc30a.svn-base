...
/**
 * Created by mdalinaser.khan on 7/22/15.
 */
@Component("UpdateApplicationUserAuthorityAction")
class UpdateApplicationUserAuthorityAction {
    @Autowired
        ...
        try{
            ...
            nAuthorityList.each {
                ...
                if (!applicationUserAuthority.validate()) {
                    return UserMessageBuilder.createMessage(applicationUserService.getUserMessage("ApplicationUser.title", null), Message.ERROR, applicationUserAuthority)
                }
    ...
    Object execute(Object params, Object object) {
        List errorList = []
        try {
            if (applicationUserService.updateAuthority((Map)object)){
                return UserMessageBuilder.createMessage(applicationUserService.getUserMessage("ApplicationUser.title", null), Message.SUCCESS, "Application User Updated Successfully")
