 ...
 */
@Component('UnlockApplicationUserAction')
class UnlockApplicationUserAction implements IAction {
    @Autowired
    ...
    Object execute(params, object) {
        List errorList = []
        try {
            if(applicationUserService.unlock((Map)object)){
        ...
        return UserMessageBuilder.createMessage(applicationUserService.getUserMessage("ApplicationUser.title", null), Message.SUCCESS, "Application User is unlocked successfully")
    }
}
