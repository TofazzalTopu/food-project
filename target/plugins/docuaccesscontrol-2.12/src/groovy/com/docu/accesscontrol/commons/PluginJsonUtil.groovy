 ...
 */
@Component("pluginJsonUtil")
class PluginJsonUtil {
...
//  }
  ...
  public List formatSqlRowResult(GroovyRowResult sqlRowResult) {
  ...
  */
    ...
    sqlResultSet.each {row ->
      userData.each { key, val ->
        if (row.containsKey(key)) {
          if (row[key].toString().isNumber()) {
            userData[key] = Double.parseDouble(val.toString()) + Double.parseDouble(row[key].toString())
          }
        }
      }

      List rowValues = formatSqlRowResult(row as GroovyRowResult)
      String data = new JSON(rowValues).toString()
      strBuffer = new StringBuffer()
  ...
  public String getJqGridFeed(String sql, Map params, boolean debug) {
    ...
    try {
      ...
      String pageSize = params.rows != null ? params.rows.toString() : (String) 10
  ...
  public String getJqGridFeedWithAddresses(String sql, Map params) {
    return getJqGridFeedWithAddresses(sql, params, false)
  }
  /**
   *
   * @param sql
  ...
  private String concatOfficeAddress(String addressLine1, String addressLine2, String addressCity) {
    String concatAddress = addressLine1 + "," + addressLine2 + "," + addressCity
